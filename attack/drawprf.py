    ...
    def generate_distinguisher_shape(self):
        ...
        for r in range(self.RB):
            if self.offset + r < 4:
                ...
                contents += trim(r"""\spnwordtobitswitch % switch from nibble view to bit view
                                    \orthrosroundnibble""") + "\n"
            x_one = [str(i) for i in range(128) if self.result[f"xb{self.branchtype}"][r][i] == 1]
            x_unknown = [str(i) for i in range(128) if self.result[f"xb{self.branchtype}"][r][i] == -1]
            if self.offset + r < 4:
                yz_one = [f"{i}/{self.bit_permutation[self.branchtype][i]}" for i in range(128) if self.result[f"yb{self.branchtype}"][r][i] == 1]
                yz_unknown = [f"{i}/{self.bit_permutation[self.branchtype][i]}" for i in range(128) if self.result[f"yb{self.branchtype}"][r][i] == -1]
            else:
                yz_one = [f"{i}/{4*self.nibble_permutation[self.branchtype][i//4] + i%4}" for i in range(128) if self.result[f"yb{self.branchtype}"][r][i] == 1]
                yz_unknown = [f"{i}/{4*self.nibble_permutation[self.branchtype][i//4] + i%4}" for i in range(128) if self.result[f"yb{self.branchtype}"][r][i] == -1]
            diff_active_sboxes = [str(i) for i in range(32) if self.result[f"ab{self.branchtype}"][r][i] >= 1]
        ...
        for r in range(self.RU):
            ...
            diff_active_sboxes = [str(i) for i in range(32) if sum(self.result[f"pu{self.branchtype}"][r][i]) >= 1]
        ...
        for r in range(self.RM):
            ...
            diff_active_sboxes = [str(i) for i in range(32) if self.result[f"dmu{self.branchtype}"][r][i] >= 1]
            lin_active_sboxes = [str(i) for i in range(32) if self.result[f"dml{self.branchtype}"][r][i] >= 1]
            common_active_sboxes = [str(i) for i in range(32) if self.result[f"dmu{self.branchtype}"][r][i] + self.result[f"dml{self.branchtype}"][r][i] == 2]
            contents += trim(r"""\orthrosmarkbits[diffone]""") + "\n"
        ...
        for r in range(self.RL):
            ...
            lin_active_sboxes = [str(i) for i in range(32) if sum(self.result[f"cl{self.branchtype}"][r][i]) >= 1]
            contents += trim(r"""\orthrosmarkbits[linone]""") + "\n"
            contents += "{{{}}}".format(",".join(x_one)) + "\n"
            contents += "{{{}}}".format(",".join(lin_active_sboxes)) + "\n"
            contents += "{{{}}}".format(",".join(yz_one)) + "\n"
