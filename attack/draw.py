...
class DrawDL():
    ...
    def __init__(self, dlobject, output_file_name="output.tex"):
        ...
        self.branchtype = dlobject.branchtype
        self.RU = dlobject.RU
        self.RM = dlobject.RM
        self.RL = dlobject.RL
    ...
    def generate_distinguisher_shape(self):
        ...
        for r in range(self.RU):
            ...
            diff_active_sboxes = [str(i) for i in range(32) if sum(self.result["pu"][r][i]) >= 1]
        ...
        for r in range(self.RM):
            ...
            diff_active_sboxes = [str(i) for i in range(32) if self.result["dmu"][r][i] >= 1]
            lin_active_sboxes = [str(i) for i in range(32) if self.result["dml"][r][i] >= 1]
            common_active_sboxes = [str(i) for i in range(32) if self.result["dmu"][r][i] + self.result["dml"][r][i] == 2]
            contents += trim(r"""\orthrosmarkbits[diffone]""") + "\n"
        ...
        for r in range(self.RL):
            if self.offset + self.RU + self.RM + r < 4:
                contents += trim(r"""\spnwordtobitswitch % switch from nibble view to bit view
                                    \orthrosroundbits""") + "\n"
            else:
                contents += trim(r"""\spnwordtobitswitch % switch from nibble view to bit view
            ...
            lin_active_sboxes = [str(i) for i in range(32) if sum(self.result["cl"][r][i]) >= 1]
            contents += trim(r"""\orthrosmarkbits[linone]""") + "\n"
            contents += "{{{}}}".format(",".join(x_one)) + "\n"
            contents += "{{{}}}".format(",".join(lin_active_sboxes)) + "\n"
            contents += "{{{}}}".format(",".join(yz_one)) + "\n"
